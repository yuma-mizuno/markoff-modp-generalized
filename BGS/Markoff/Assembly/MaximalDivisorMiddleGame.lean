...
import BGS.Markoff.MiddleGame.MaximalDivisorCorvajaZannierEscape

/-!
# Maximal-divisor middle-game assembly

This module lifts the maximal-order Corvaja--Zannier escape from a chosen
coordinate to the maximum of the three coordinate rotation orders.
-/
...
/-- One middle-game step strictly increases the maximum coordinate order when
the two size inequalities use only divisibility-maximal candidate orders. -/
theorem exists_sameNormalizedComponent_maximalOrder_increase_of_maximalDivisorBounds
  ...
  have hx'Linear :
      corvajaZannierCorollaryTwoSafeCoefficient *
        (middleGameMaximalOrders p (rotationOrder x'.1.u1)).card *
  ...
  have hbase :
      4 < (corvajaZannierCorollaryTwoSafeCoefficient *
        (middleGameMaximalOrders p (rotationOrder x'.1.u1)).card) ^ 3 := by
    have hcoeff : corvajaZannierCorollaryTwoSafeCoefficient ≤
        corvajaZannierCorollaryTwoSafeCoefficient *
          (middleGameMaximalOrders p (rotationOrder x'.1.u1)).card := by
  ...
  have hx'Nonparabolic : x'.1.u1 ^ 2 ≠ 4 := by
    ...
    · rw [hnegTwo, rotationOrder_neg_two p hpTwo] at hx'Below
      ...
      linarith
  obtain ⟨n, hnIncrease⟩ :=
    exists_iterate_with_larger_secondRotationOrder_of_weightedTraceBound_maximalOrders
      p hpTwo delta hdelta x'.1 x'.property hx'0 hx'Nonparabolic hx'Below
