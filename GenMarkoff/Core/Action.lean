...
The source note defines its graph using the even words
`Rᵢ = μᵢ₋₁ ∘ μᵢ₊₁`, while arXiv:2509.02187 proves divisibility for orbits of
the individual Vieta moves.  Both actions are therefore kept as distinct
objects.
-/
...
/-- The three even-word rotations used as graph edges in `markov_modp.tex`. -/
...
/-- The subgroup generated by the source note's three rotations. -/
...
/-- The first source rotation belongs to the fixed-coefficient rotation group. -/
...
/-- The second source rotation belongs to the fixed-coefficient rotation group. -/
...
/-- The third source rotation belongs to the fixed-coefficient rotation group. -/
...
/-- The first source rotation on the punctured fixed-coefficient surface. -/
...
/-- The second source rotation on the punctured fixed-coefficient surface. -/
...
/-- The third source rotation on the punctured fixed-coefficient surface. -/
...
/-- The orbit of a punctured point under the three source rotations. -/
